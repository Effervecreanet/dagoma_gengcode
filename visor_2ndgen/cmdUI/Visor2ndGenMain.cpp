...
#include "Visor2ndGenDefs.h"

#pragma comment(lib, "user32.lib")


DWORD64 total_gcode_lines = VISORGEN2_TOTAL_GCODE_LINES;
...
static int
get_user_input(HANDLE consOut, HANDLE consInp, COORD cursPos, CHAR dataInput[10], enum dataType dt)
{
    ...
    for (i = 0, cursPos.X = 4 + user_info_strlen;;) {
        ...
        if (GetNumberOfConsoleInputEvents(consInp, &read) &&
            read > 0 &&
            ...
            if (inputRecord[0].EventType == KEY_EVENT &&
                ...
                if ((dataInput[0] == '-' && strchr(&dataInput[1], '-') != NULL) ||
                   ...
                   ((dt == FW && (atof(dataInput) < 80  || atof(dataInput) > 600)))) {
                    ...
                    WriteConsole(consOut, USER_INFO_STRING8, sizeof(USER_INFO_STRING8) - 1, &written, NULL);
                    Sleep(4000);
                    SetConsoleCursorPosition(consOut, cursPos);
                    WriteConsole(consOut, "                              ", 30, &written, NULL);
                    return -1;
                }
                else {
                    ReadConsoleInput(consInp, inputRecord, 7, &read);
            ...
            else if (inputRecord[0].EventType == KEY_EVENT && inputRecord[0].Event.KeyEvent.bKeyDown) {
...
int main(int argc, char** argv)
{
    ...
    CHAR cBufferFirmware[4096];
    DWORD j, percentStep = 1;
    PCHAR pStrFirmware;
    struct pBar pbar;
...
clear:
    ...
    WriteConsole(hConsStdOut, USER_INFO_STRING1, sizeof(USER_INFO_STRING1) - 1, &written, NULL);
    ...
    cursPos.X = 4;
    cursPos.Y = 11;
    SetConsoleTextAttribute(hConsStdOut, FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE);
    SetConsoleCursorPosition(hConsStdOut, cursPos);
    WriteConsole(hConsStdOut, USER_INFO_STRING3, sizeof(USER_INFO_STRING3) - 1, &written, NULL);

    cursPos.X = 4;
    cursPos.Y = 12;
    SetConsoleCursorPosition(hConsStdOut, cursPos);
    WriteConsole(hConsStdOut, USER_INFO_STRING9, sizeof(USER_INFO_STRING9) - 1, &written, NULL);

    cursPos.X = 4;
    cursPos.Y = 13;
    SetConsoleCursorPosition(hConsStdOut, cursPos);
    WriteConsole(hConsStdOut, USER_INFO_STRING4, sizeof(USER_INFO_STRING4) - 1, &written, NULL);

    cursPos.X = 4;
    cursPos.Y = 14;
    SetConsoleCursorPosition(hConsStdOut, cursPos);
    WriteConsole(hConsStdOut, USER_INFO_STRING5, sizeof(USER_INFO_STRING5) - 1, &written, NULL);

    cursPos.X = 4;
    cursPos.Y = 16;
    SetConsoleCursorPosition(hConsStdOut, cursPos);
    WriteConsole(hConsStdOut, USER_INFO_STRING2, sizeof(USER_INFO_STRING2) - 1, &written, NULL);
    ...
    cursPos.Y = 17;
    SetConsoleCursorPosition(hConsStdOut, cursPos);
    WriteConsole(hConsStdOut, USER_INFO_STRING10, sizeof(USER_INFO_STRING10) - 1, &written, NULL);
    ...
    cursPos.Y = 18;
    SetConsoleCursorPosition(hConsStdOut, cursPos);
    WriteConsole(hConsStdOut, USER_INFO_STRING11, sizeof(USER_INFO_STRING11) - 1, &written, NULL);
    ...
    hNamedPipe = CreateNamedPipe(STR_PIPE_NAME,
        PIPE_ACCESS_DUPLEX,
        PIPE_WAIT,
        2,
        255,
    ...
    hThrdObj3d = CreateThread(NULL,
        0,
        (LPTHREAD_START_ROUTINE)funcThreadGenGCODE,
        NULL,
        0,
        &dwThrdId);
    ...
    for (current_gcode_lines = 0, cursPos.Y = 13, cursPos.X = 4 + sizeof(USER_INFO_STRING4) - 1;;) {
        ...
        if (++current_gcode_lines > ((total_gcode_lines / 100) * pbar.percentStep)) {
            CHAR strPercent[7];
    ...
    cursPos.Y = 15;
    SetConsoleCursorPosition(hConsStdOut, cursPos);
    WriteConsole(hConsStdOut, USER_INFO_STRING7, sizeof(USER_INFO_STRING7) - 1, &written, NULL);
